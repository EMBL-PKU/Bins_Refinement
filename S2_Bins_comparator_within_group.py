        ...
        try:
            ...
            for line in f:
                ...
                if '_genomes.0' not in genome_ids:
                    ...
                    try:
                        if line.strip().count(':') > 5:
                            contamination=str(line).strip().split('\'Contamination\': ')[1].split(', ')[0]
                        elif line.strip().count(':') == 5:
                            contamination=str(line).strip().split('\'Contamination\': ')[1].split('}')[0]
                        else:
                            continue
                        bins_checkm[genome_ids]['Contamination']=float(contamination)
                    except:
                        bins_checkm[genome_ids]['Contamination']=0
...
def two_groups_comparator(binset1, binset2, num):
    pwd=os.getcwd()
    print 'Iteration', num
...
def bin_within_a_group_comparitor(binset, assembly, num):
    ...
    for line in f:
        ...
        connections=int(str(line).strip().split('Connections\': ')[1].split(',')[0])
        ...
        genome_size=int(str(line).strip().split('\'Genome size\': ')[1].split(',')[0])
        if '_maxbin2_genomes.' in ids or '_concoct_genomes.' in ids:
            bin_id=ids+'.fasta'
        elif '_metabat_genomes.' in ids:
            bin_id=ids+'.fa'
        else:
            continue 
        final_iteration_checkm[bin_id]={'Connections': connections, 'marker lineage': marker_lineage, 'Completeness': completeness, 'Genome size': genome_size, 'Contamination': contamination}
...
def bins_comparator_multiple_groups(genome_folder, assembly):
    ...
    for num in  range(0,num_binset-1):
        num+=1
        if num == 1:
            two_groups_comparator(str(genome_folder[0]).split('_genomes')[0], str(genome_folder[-1]).split('_genomes')[0], num)
        else:
            genome_folder.remove(genome_folder[0])
            genome_folder.remove(genome_folder[-1])
            genome_folder.append('Iteration_'+str(num-1))
            two_groups_comparator(str(genome_folder[-1]).split('_genomes')[0], str(genome_folder[0]).split('_genomes')[0], num)
    i=0
...
if __name__ == '__main__': 
    bins_folders_name_list=['3_ZH_MO_final.contigs.fa_0.3_maxbin2_genomes', '3_ZH_MO_final.contigs.fa_0.5_maxbin2_genomes', '3_ZH_MO_final.contigs.fa_0.7_maxbin2_genomes', '3_ZH_MO_final.contigs.fa_0.9_maxbin2_genomes', '3_ZH_MO_final.contigs.fa_200_metabat_genomes', '3_ZH_MO_final.contigs.fa_300_metabat_genomes', '3_ZH_MO_final.contigs.fa_400_metabat_genomes', '3_ZH_MO_final.contigs.fa_500_metabat_genomes', '3_ZH_MO_final.contigs.fa_200_concoct_genomes', '3_ZH_MO_final.contigs.fa_400_concoct_genomes']
    # depth_file='assembly.depth.txt'
    assembly='3_ZH_MO_final.contigs.fa'
